...
RustLink is inspired by [NodeLink v3](https://github.com/PerformanC/NodeLink) (AGPL-3.0) but is an independent implementation of the Lavalink v4 API — no NodeLink source was copied.

**External runtime dependency:** YouTube SABR fragmented MP4 decoding requires `ffmpeg` (`apt install ffmpeg` / `brew install ffmpeg`). It is spawned as an external binary (`ffmpeg -i pipe:0 -f f32le -ac 2 -ar 48000 pipe:1`) and not linked, so MIT is preserved.
