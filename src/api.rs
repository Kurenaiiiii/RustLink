...
async fn version(State(state): State<SharedState>, headers: HeaderMap) -> Response {
    ...
    ([(header::CONTENT_TYPE, "text/plain")], env!("CARGO_PKG_VERSION")).into_response()
}
...
async fn patch_player(
    ...
    headers: HeaderMap,
    Path((session_id, guild_id)): Path<(String, String)>,
    payload_result: Result<Json<PatchPlayerPayload>, JsonRejection>,
) -> Response {
    state.increment_api_request("/v4/sessions/:id/players/:id");
    if let Err(response) = require_auth(&headers, &state) {
        return response;
    ...
    // Surface the real parse error instead of letting Axum's default
    // plain-text 400 through — Shoukaku (and other Lavalink clients)
    // expect the standard Lavalink error JSON shape and will otherwise
    // fail with a generic "Unexpected error response from Lavalink server".
    let Json(payload) = match payload_result {
        Ok(p) => p,
        Err(rejection) => {
            error!(
                target: "API",
                "PATCH /sessions/{}/players/{} body rejected: {}",
                session_id, guild_id, rejection
            );
            return (
                StatusCode::BAD_REQUEST,
                Json(json!({
                    ...
                    "path": format!("/{}/sessions/{}/players/{}", crate::constants::PATH_VERSION, session_id, guild_id)
                })),
            )
                .into_response();
        }
    ...
    // Build commands for every field the client sent — Shoukaku and other
    // Lavalink v4 clients drive pause/volume/filters exclusively through REST
    // PATCH, so each field MUST be forwarded to the audio worker.
    let mut cmds: Vec<WorkerCommand> = Vec::new();
    if let Some(ref voice) = payload.voice {
        state.plugin_manager.on_voice_server_update(&guild_id, &voice.endpoint, &voice.token).await;
        cmds.push(WorkerCommand::VoiceUpdate {
            session_id: voice.session_id.clone(),
            user_id: user_id.clone(),
            token: voice.token.clone(),
            endpoint: voice.endpoint.clone(),
            channel_id: voice.channel_id.clone().unwrap_or_else(|| "0".into()),
        });
    }
    if let Some(ref track_val) = payload.track {
        if let Some(encoded) = track_val.get("encoded").and_then(|e| e.as_str()) {
            cmds.push(WorkerCommand::Play {
    ...
    if let Some(paused) = payload.paused {
        cmds.push(WorkerCommand::Pause(paused));
    }
    if let Some(volume) = payload.volume {
        cmds.push(WorkerCommand::Volume(volume.min(1000) as u16));
    }
    if let Some(ref filters) = payload.filters {
        cmds.push(WorkerCommand::Filters(filters.clone()));
    }
    ...
    if let Some(player) = session.players.iter_mut().find(|p| p.guild_id == guild_id) {
        ...
        if let Some(ref voice) = payload.voice {
            player.voice = crate::state::PlayerVoiceState {
                token: Some(voice.token.clone()),
                endpoint: Some(voice.endpoint.clone()),
                session_id: Some(voice.session_id.clone()),
                channel_id: voice.channel_id.clone(),
            };
        }
    } else {
        let voice_state = if let Some(ref voice) = payload.voice {
            crate::state::PlayerVoiceState {
                token: Some(voice.token.clone()),
                endpoint: Some(voice.endpoint.clone()),
                session_id: Some(voice.session_id.clone()),
                channel_id: voice.channel_id.clone(),
    ...
    // Ensure worker exists and send commands (voice first, then playback,
    // then state updates).
    if !cmds.is_empty() {
