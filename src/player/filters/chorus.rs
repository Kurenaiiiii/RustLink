...
impl ChorusFilter {
    pub fn new() -> Self {
        let max_delay = (0.05 * SAMPLE_RATE) as usize;
        Self {
            priority: PRIORITY_DEFAULT,
            config: AnimatableConfig::new(&[0.02, 0.003, 0.5]),
...
impl AnimatableFilter for ChorusFilter {
    ...
    fn update(&mut self, settings: &Value) {
        self.config.apply_animated_update(settings, "chorus", &[("delay", 0), ("depth", 1), ("rate", 2)]);
    ...
    fn process(&mut self, chunk: &mut [f32], _channels: usize, sample_rate: f32) {
        ...
        let delay = c[0].clamp(0.001, 0.05);
        let depth = c[1].clamp(0.0, 0.01);
        ...
        for frame in chunk.chunks_mut(2) {
            ...
            for ch in 0..2 {
                ...
                out[ch] = clamp_sample(frame[ch] + delayed * 0.5);
            }
            frame[0] = out[0];
            frame[1] = out[1];
