...
use super::{PRIORITY_DEFAULT, BiquadState, lowpass_coeffs};
...
impl LowPassFilter {
    pub fn new() -> Self {
        Self {
            ...
            config: AnimatableConfig::new(&[20000.0]),
...
impl AnimatableFilter for LowPassFilter {
    ...
    fn update(&mut self, settings: &Value) {
        self.config.apply_animated_update(settings, "lowpass", &[("frequency", 0)]);
        self.freq = self.config.get_current()[0].clamp(10.0, 24000.0);
        self.active = self.freq < 20000.0;
    }
    ...
    fn process(&mut self, chunk: &mut [f32], _channels: usize, sample_rate: f32) {
        ...
        let (b0, b1, b2, a1, a2) = lowpass_coeffs(self.freq);
