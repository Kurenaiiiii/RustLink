...
pub struct PlayerWorker {
    ...
    // Keep VoiceConnection alive to maintain WS heartbeat
    voice_connection: Option<VoiceConnection>,
}
...
impl PlayerWorker {
    ...
    async fn start_track(
        ...
        let user_id = voice_session.user_id;
        ...
        let encryption_mode = voice_session.encryption_mode;
        let dave_session = voice_session.dave_session.clone();
        ...
        let playback_handle = tokio::spawn(async move {
            ...
            info!(target: "Worker", "Starting playback ssrc={}, addr={}, mode={:?} (guild: {})", ssrc, addr, encryption_mode, guild_id);
            ...
            loop {
                ...
                while tape_buffer.len() >= frame_samples {
                    ...
                    if let Err(e) = sess.send_opus_frame(&encoded).await {
                        ...
                        sent_count += 1;
                        if sent_count % 50 == 0 {
                            info!(target: "Worker", "Audio frames sent: {} (guild: {})", sent_count, guild_id);
                        }
                ...
                cmd = self.rx.recv() => {
                    ...
                    match cmd {
                            ...
                            match conn.connect().await {
                                Ok(session_arc) => {
                                    ...
                                    let session = VoiceSession {
                                        ...
                                        dave_session: guard.dave_session.clone(),
                                    };
                                    drop(guard);
                                    {
                                    ...
                                    self.voice_connection = Some(conn);
                        ...
                        WorkerCommand::Play { encoded_track, no_replace } => {
                            ...
                            // Refresh session from VoiceConnection (WS may have reconnected with new key)
                            if let Some(ref conn) = self.voice_connection {
                                let session_arc = conn.get_session();
                                let guard = session_arc.lock().await;
                                if let Some(ref vs) = *guard {
                                    voice_session = Some(vs.clone());
                                }
                            }
