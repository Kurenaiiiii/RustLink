...
async fn handle_socket(
    ...
    // Cleanup when done — respect resuming
    {
        let mut ws_senders = state.ws_senders.write().await;
        ws_senders.remove(&session_id);
    }
    let should_keep = state.sessions.get(&session_id).map(|s| s.resuming).unwrap_or(false);
    if should_keep {
        let timeout = state.sessions.get(&session_id).map(|s| s.timeout).unwrap_or(60);
        let state_clone = state.clone();
        let sid = session_id.clone();
        info!(target: "WebSocket", "Session {} is resumable, keeping for {}s", sid, timeout);
        tokio::spawn(async move {
            tokio::time::sleep(tokio::time::Duration::from_secs(timeout)).await;
            if !state_clone.ws_senders.read().await.contains_key(&sid) {
                state_clone.sessions.remove(&sid);
                info!(target: "WebSocket", "Resumable session {} expired after {}s", sid, timeout);
            }
        });
    } else {
