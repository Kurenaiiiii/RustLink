...
impl SourceRegistry {
    ...
    pub async fn resolve(&self, query: &str) -> anyhow::Result<SourceResult> {
        let providers = self.providers.read().await;
        // Try every provider before giving up. A provider returning `Error`
        // means "I recognized this well enough to try, but failed" (e.g. a
        // service needing an unconfigured token) — that must NOT stop other
        // providers (including the one that actually owns this URL/domain)
        // from getting their turn. Only a genuine match (Track/Playlist/
        // Search) short-circuits the loop. If nothing matches, surface the
        // last real error seen (more useful than a bare "not found"), or
        // Empty if no provider errored either.
        let mut last_error: Option<SourceResult> = None;
        for provider in providers.iter() {
            if let Ok(res) = provider.resolve(query, None).await {
                match res {
                    SourceResult::Empty => continue,
                    SourceResult::Error(_) => {
                        last_error = Some(res);
                        continue;
                    }
                    _ => return Ok(res),
        ...
        Ok(last_error.unwrap_or(SourceResult::Empty))
    }
