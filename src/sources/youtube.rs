...
const INNERTUBE_API_KEY: &str = "AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8"; // pragma: allowlist secret - public YouTube InnerTube key, same as NodeLink/yt-dlp, not a private secret
...
impl YouTubeSource {
    ...
    pub fn start_background_tasks(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                potoken.refresh_visitor_data().await;
            }
        });
    ...
    async fn innertube_with_client(
        ...
        // Inject poToken if available — content-bound when the payload targets
        // a video (NodeLink mints per videoId), session-bound otherwise.
        let video_id = payload
            .get("videoId")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        if let Some(token) = self.potoken.token_for_player_request(video_id.as_deref()).await {
            if let Some(obj) = body.as_object_mut() {
        ...
        // Identity headers NodeLink sends on EVERY Innertube request: the
        // per-client User-Agent plus visitor/client identification. YouTube
        // cross-checks these against the body context; when they are missing,
        // web-family clients answer with LOGIN_REQUIRED / UNPLAYABLE bot-checks.
        let mut req = self.client.post(&url).json(&body);

        req = req
            .header("User-Agent", client_kind.user_agent())
            .header("X-Goog-Visitor-Id", &vd)
            .header(
                "X-Youtube-Client-Name",
                client_kind.client_id().to_string(),
            )
            .header("X-Youtube-Client-Version", client_kind.client_version());

        match client_kind {
            ClientKind::WebRemix | ClientKind::Music => {
                req = req
                    .header("Origin", "https://music.youtube.com")
                    .header("Referer", "https://music.youtube.com");
            }
            ClientKind::Web | ClientKind::WebEmbedded | ClientKind::WebParentTools => {
                req = req
        ...
        // Per-client extras, skipping keys already set above.
        for (key, value) in client_kind.search_headers() {
            if key.eq_ignore_ascii_case("x-youtube-client-name")
                || key.eq_ignore_ascii_case("x-youtube-client-version")
            {
                continue;
            }
    ...
    fn extract_sabr_config(
        ...
        // NodeLink reads the ustreamer config from
        // playerConfig.mediaCommonConfig.mediaUstreamerRequestConfig; older
        // layouts nested it directly under streamingData.
        let ustreamer_field = data
            .pointer("/playerConfig/mediaCommonConfig/mediaUstreamerRequestConfig/videoPlaybackUstreamerConfig")
            .or_else(|| streaming_data.get("videoPlaybackUstemarerConfig"))
            .or_else(|| streaming_data.get("videoPlaybackUstreamerConfig"));
        let ustreamer_config_bytes = ustreamer_field
            .and_then(|v| v.as_str())
            .and_then(|s| {
                let s_clean = s.replace('-', "+").replace('_', "/");
                let mod_len = s_clean.len() & 3;
                let padded = if mod_len != 0 {
                    format!("{}{}", s_clean, "=".repeat(4 - mod_len))
                } else {
                    s_clean
                };
        ...
        // NodeLink's Android/ Web SABR path sends ALL formats (formats +
        // adaptiveFormats merged, URLs or not) whenever serverAbrStreamingUrl
        // exists. The ABR server picks from preferredFormatIds. Filtering to
        // URL-less or audio-only made SABR unreachable on AndroidVR (all 22
        // have URLs) and forced progressive-GET fallback.
        let sabr_formats: Vec<serde_json::Value> = {
            let mut raw: Vec<&Value> = Vec::new();
            if let Some(arr) = streaming_data.get("adaptiveFormats").and_then(|v| v.as_array()) {
                raw.extend(arr.iter());
            }
            if let Some(arr) = streaming_data.get("formats").and_then(|v| v.as_array()) {
                raw.extend(arr.iter());
            }
            // snake_case fallbacks (some InnerTube dumps)
            if let Some(arr) = streaming_data.get("adaptive_formats").and_then(|v| v.as_array()) {
                raw.extend(arr.iter());
            }
            raw.iter()
                .filter(|f| f.get("itag").and_then(|v| v.as_u64()).is_some())
                .map(|f| {
                    let itag = f.get("itag").and_then(|v| v.as_u64()).unwrap_or(0) as i32;
                    json!({
                        "itag": itag,
                        "mimeType": f.get("mimeType").or_else(|| f.get("mime_type")).and_then(|v| v.as_str()),
                        "xtags": f.get("xtags").and_then(|v| v.as_str()),
                        "lastModified": f.get("lastModified").or_else(|| f.get("last_modified_ms")).or_else(|| f.get("lmt")).and_then(|v| v.as_str()),
                        "audioTrackId": f.get("audioTrackId").and_then(|v| v.as_str()).or_else(|| f.get("audioTrack").and_then(|at| at.get("id")).and_then(|v| v.as_str())),
                        "bitrate": f.get("bitrate").or_else(|| f.get("averageBitrate")).or_else(|| f.get("average_bitrate")).and_then(|v| v.as_i64()),
                    })
                })
                .collect()
        };
        ...
        let client_version = client
            .sabr_client_version()
            .or_else(|| Some(client.client_version()))
            .unwrap_or("1.0");

        let ustreamer_b64 = ustreamer_config_bytes.as_ref().map(|b| {
            use base64::Engine;
            base64::engine::general_purpose::STANDARD
                .encode(b)
                .replace('+', "-")
        ...
        let mut sabr_info = serde_json::Map::new();
        sabr_info.insert(
            "serverAbrStreamingUrl".into(),
            json!(server_abr_url),
        );
        sabr_info.insert(
            "clientName".into(),
        ...
        if let Some(ref pt) = po_token {
            // Pass the minted websafe token through verbatim — the worker
            // decodes it once into raw bytes for the streamer context.
            sabr_info.insert("poToken".into(), json!(pt));
        }
        if !visitor_data.is_empty() {
            sabr_info.insert("visitorData".into(), json!(visitor_data));
        }
        if let Some(ref ub64) = ustreamer_b64 {
            sabr_info.insert("ustreamerConfig".into(), json!(ub64));
        }

        Some(TrackUrlResult {
            url: Some(server_abr_url.to_string()),
            protocol: Some("sabr".into()),
            format: json!("sabr"),
            new_track: None,
            additional_data: json!({ "sabr": sabr_info }),
            exception: None,
        })
    }
    ...
    /// Cheap ranged probe that verifies a stream URL is actually downloadable.
    ///
    /// YouTube's PO-token enforcement (2025+) makes many Innertube clients
    /// hand out videoplayback URLs that 403 on download — the player response
    /// looks perfectly healthy. The only way to know is to try: fetch the
    /// first 2 KB with `Range` and require 200/206.
    #[allow(dead_code)]
        ...
        match req.send().await {
            Ok(resp) => {
                ...
                if status == reqwest::StatusCode::OK || status == reqwest::StatusCode::PARTIAL_CONTENT {
                    ...
                    warn!(target: "YouTube", "Stream probe rejected: HTTP {status} — trying next client");
                    false
                }
            }
            Err(e) => {
                warn!(target: "YouTube", "Stream probe failed: {e} — trying next client");
                false
            }
        }
    }
...
impl SourceProvider for YouTubeSource {
    ...
    async fn get_track_url(&self, track: &TrackInfo) -> anyhow::Result<TrackUrlResult> {
        ...
        // Mint tokens once up front: content tokens ride in player requests
        // and SABR streamer contexts; the session token is appended as `pot`.
        let session_token = self.potoken.try_generate().await;
        let visitor_data = self.potoken.visitor_data();

        // Single pass, NodeLink-speed: the FIRST client that yields a usable
        // stream wins immediately. No second pass, no dead-client tax.
        for &client in &self.enabled_clients {
                ...
                .await
            {
                Ok(d) => d,
                Err(e) => {
                    last_error = format!("{} request error: {e}", client.display_name());
                    warn!(target: "YouTube", "{}", last_error);
                    continue;
                }
            };

            if let Some(reason) = check_playability(&data) {
                last_error = format!("{}: {}", client.display_name(), reason);
                warn!(target: "YouTube", "{}", last_error);

                // LOGIN_REQUIRED + OAuth configured → one TV retry.
                if reason.contains("LOGIN_REQUIRED") && !tried_login && self.oauth.is_some() {
                    tried_login = true;
                    if let Some(info) =
                        self.try_player_with_auth(video_id, ClientKind::Tv).await
                    {
                        return Ok(TrackUrlResult {
                            ...
                            additional_data: json!({
                                "stream_headers": ClientKind::Tv.stream_headers()
                            }),
                            exception: None,
                        });
                    }
            ...
            // SABR first within this client (carries our poToken).
            let sabr_po_token = self
                .potoken
                .token_for_player_request(Some(video_id))
                .await
                .or_else(|| session_token.clone());
            if let Some(sabr_result) =
                self.extract_sabr_config(&data, &client, sabr_po_token, &visitor_data)
            {
                return Ok(sabr_result);
            }

            // Progressive fallback from this same client.
            let needs_cipher = client.requires_player_script();
            if let Some(info) = self.extract_best_audio_url(&data, needs_cipher).await {
                // NOTE: no probe — touching a single-use URL twice kills it.
                let mut url = info.url;
                if let Some(token) = &session_token {
                    let sep = if url.contains('?') { '&' } else { '?' };
                    url = format!("{url}{sep}pot={token}");
                }
                ...
                return Ok(TrackUrlResult {
                    ...
                    new_track: None,
                    additional_data: json!({
                        "stream_headers": client.stream_headers()
                    }),
                    exception: None,
                });
            }
