...
impl ClientKind {
    ...
    pub fn client_version(&self) -> &'static str {
        match self {
            ClientKind::Web => "2.20260821.01.00",
            ...
            ClientKind::Tv => "7.20260820.16.00",
    ...
    /// Headers that must accompany HTTP requests to the stream URLs this
    /// client produced. googlevideo.com rejects (or hangs / returns empty
    /// bodies to) requests whose User-Agent doesn't match the Innertube
    /// client that negotiated the URL.
    pub fn stream_headers(&self) -> serde_json::Value {
        let mut headers = serde_json::Map::new();
        headers.insert("User-Agent".into(), json!(self.user_agent()));
        match self {
            ClientKind::Web | ClientKind::WebEmbedded | ClientKind::WebParentTools | ClientKind::WebRemix => {
                headers.insert("Referer".into(), json!("https://www.youtube.com/"));
                headers.insert("Origin".into(), json!("https://www.youtube.com"));
            }
            _ => {}
        }
        json!(headers)
