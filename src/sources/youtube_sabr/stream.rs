...
impl UmpReader {
    ...
    fn read_varint(&self, offset: usize) -> (i64, usize) {
        ...
        let value = match byte_length {
            1 => {
                let v = self.composite_buffer.get_uint8(off) as i64;
                off += 1;
                v
            }
            2 => {
                ...
                (b1 as i64 & 0x3f) + 64 * b2 as i64
            }
            3 => {
                let b1 = self.composite_buffer.get_uint8(off); off += 1;
                let b2 = self.composite_buffer.get_uint8(off); off += 1;
                let b3 = self.composite_buffer.get_uint8(off); off += 1;
                (b1 as i64 & 0x1f) + 32 * (b2 as i64 + 256 * b3 as i64)
            }
            4 => {
                let b1 = self.composite_buffer.get_uint8(off); off += 1;
                let b2 = self.composite_buffer.get_uint8(off); off += 1;
                ...
                (b1 as i64 & 0x0f) + 16 * (b2 as i64 + 256 * (b3 as i64 + 256 * b4 as i64))
            }
            _ => {
                off += 1;
                let b1 = self.composite_buffer.get_uint8(off); off += 1;
                ...
                let b3 = self.composite_buffer.get_uint8(off); off += 1;
                let b4 = self.composite_buffer.get_uint8(off); off += 1;
                b1 as i64 + 256 * (b2 as i64 + 256 * (b3 as i64 + 256 * b4 as i64))
            }
        };
        (value, off)
    }
...
impl SabrStream {
    ...
    async fn fetch_and_process_segments(
        ...
        if self.video_playback_ustreamer_config.is_none() {
            warn!("SABR ustreamer config missing, proceeding without it");
        }
        if self.client_info.is_none() {
            warn!("SABR client info missing, proceeding without it");
        }
