...
impl AppState {
    pub async fn new(config: NodeLinkConfig) -> SharedState {
        ...
        if config.sources.youtube.enabled {
            // ── Resolve YouTube auth material (NodeLink parity) ─────────────
            // Priority: explicit refresh_tokens → clients.settings.TV
            // → interactive device flow when get_oauth_token = true.
            let mut refresh_tokens = config.sources.youtube.refresh_tokens.clone();
            if refresh_tokens.is_empty() {
                if let Some(settings) = &config.sources.youtube.clients.settings {
                    if let Some(tv) = settings.get("TV") {
                        if let Some(arr) = tv.get("refreshToken").and_then(|v| v.as_array()) {
                            for t in arr {
                                if let Some(t) = t.as_str() {
                                    if !t.is_empty() {
                                        refresh_tokens.push(t.to_string());
                                    }
                                }
                            }
                        }
            ...
            if refresh_tokens.is_empty() && config.sources.youtube.get_oauth_token {
                crate::logging::started(
                    "YouTube",
                    "No refresh tokens configured — starting device authorization.".into(),
                );
                crate::logging::started(
                    "YouTube",
                    "⚠ Use a SECONDARY/throwaway Google account, not your main one!".into(),
                );
                match crate::sources::youtube_oauth::YouTubeOAuth::acquire_refresh_token().await {
                    Ok(token) => {
                        crate::logging::started(
                            "YouTube",
                            format!(
                                "Authorized! Add this to [sources.youtube] refresh_tokens to persist: {token}"
                            ),
                        );
                        refresh_tokens.push(token);
                    }
                    Err(e) => {
                        tracing::warn!(target: "YouTube", "Device authorization failed: {e}");
                    }
                }
            }

            if refresh_tokens.is_empty()
                && config.sources.youtube.potoken.is_none()
                && config.sources.youtube.po_token_endpoint.is_none()
            {
                tracing::warn!(
                    target: "YouTube",
                    "No YouTube auth configured — playback will likely fail with \
                     'Sign in to confirm you're not a bot'. Fix: set \
                     sources.youtube.get_oauth_token = true and restart (follow console), \
                     or configure potoken / po_token_endpoint."
                );
            }
        ...
        // HttpProvider is a generic direct-link fallback (raw .mp3/.ogg/etc URLs) —
        // it must be registered LAST so domain-specific sources (SoundCloud, Spotify,
        // YouTube, etc.) always get first chance to resolve a URL. Otherwise it
        // intercepts every http(s) link, HEAD-requests it, and errors out on any
        // page that isn't a raw media file (e.g. a soundcloud.com track page returns
        // text/html), silently shadowing the correct source.
        if config.sources.http.enabled {
            sources.register(HttpProvider::default()).await;
        }
        // GoogleTtsProvider.resolve() accepts ANY non-empty text unconditionally
        // (that's the nature of text-to-speech — it has no way to say "this isn't
        // mine"), so it must be registered dead last, after even HttpProvider.
        // Otherwise it swallows every URL nobody else claimed yet and "resolves"
        // it into a clip of Google TTS reading the URL out loud instead of letting
        // later/legit providers (or a real "not found") happen.
