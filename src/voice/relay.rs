...
use crate::player::voice::{DaveHandle, EncryptionMode};
use davey::MediaType;
    ...
    pub fn start_listener(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // DAVE inner decrypt (if session is ready)
                let decrypted = if let Some(ref dh) = dave_handle {
                    let mut ds = dh.lock().await;
                    if let Some(ref mut session) = *ds {
                        if session.is_ready() {
                            session.decrypt(user_id, MediaType::AUDIO, &decrypted)
                                .unwrap_or_else(|e| {
                                    warn!(target: "VoiceRelay", "DAVE decrypt failed: {e}");
                                    decrypted
                                })
                        } else {
                            decrypted
